    ...
    return {
      ...
      fix: (fixer: RuleFixer) => {
        if (blockStatement.body.length === 0) {
          const newRange: TSESTree.Range = [blockStatement.range[0] + 1, blockStatement.range[1]]
...
const isSetterLikeMethodDefinition = (node: TSESTree.MethodDefinition, functionName: string) => {
